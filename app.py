...
def compute_snr(image_data, noisy_data):
    signal = np.mean(image_data ** 2)
    noise = np.mean((image_data - noisy_data) ** 2)
...
def compress_image(image_path, k=16):
    ...
    snr_original = compute_snr(img_data, img_data)  # SNR of original (trivially high)
    snr_compressed = compute_snr(img_data, compressed_img_data)
    return compressed_image_path, snr_original, snr_compressed
...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            file.save(filepath)
            compressed_path, snr_original, snr_compressed = compress_image(filepath)
            return render_template('result.html', 
                                   image_url=url_for('get_file', filename=os.path.basename(compressed_path)),
                                   snr_original=snr_original, snr_compressed=snr_compressed)
